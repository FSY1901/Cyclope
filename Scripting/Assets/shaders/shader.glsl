...
vec3 calcDirLight(DirectionalLight light, vec3 normal, vec3 viewDir){
    // ambient
    vec3 ambient = light.ambient * (material.diffuse * vec3(texture(material.diffuseMap, TexCoord)));
  	
    // diffuse 
    vec3 lightDir = normalize(-light.direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = light.diffuse * (diff * material.diffuse * vec3(texture(material.diffuseMap, TexCoord)));
    
    // specular
    vec3 reflectDir = reflect(-lightDir, normal);  
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    vec3 specular = light.diffuse * (spec * material.specular * vec3(texture(material.specularMap, TexCoord)));  
...
vec3 calcSpotLight(SpotLight light, vec3 normal, vec3 frag, vec3 viewDir){
    ...
    // diffuse 
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = light.diffuse * diff * material.diffuse * texture(material.diffuseMap, TexCoord).rgb;  
        
    // specular
    vec3 reflectDir = reflect(-lightDir, normal);  
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    vec3 specular = light.diffuse * spec * material.specular * texture(material.specularMap, TexCoord).rgb;
